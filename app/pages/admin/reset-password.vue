<template>
  <div class="min-h-screen flex items-center justify-center bg-primary-900 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
    <!-- Animated background (matches login / forgot-password) -->
      ...
      >
        ...
        >
          <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-primary-600/20 mb-5 ring-1 ring-primary-600/30 animate-icon-glow">
            <Icon name="mdi:lock-reset" class="w-8 h-8 text-primary-600" />
          </div>
        ...
        >
          <div v-if="success" class="text-center space-y-4">
            ...
            <NuxtLink
              to="/admin/login"
              class="inline-flex items-center justify-center gap-2 w-full bg-primary-600 hover:bg-primary-500 text-white font-semibold py-3 rounded-xl transition-all duration-200 shadow-lg shadow-primary-600/30 hover:shadow-primary-600/50"
            >
...
// Redirect if missing token/email, otherwise arm the entrance animation.
onMounted(() => {
  if (!token || !email) {
