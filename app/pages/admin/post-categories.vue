<template>
  <div class="max-w-4xl mx-auto">
    <!-- Page Header -->
    <AdminPageHeader title="Blog Categories" subtitle="Organize posts into topics with color-styled tiles">
      <template #breadcrumb>
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
          <NuxtLink to="/admin" class="hover:text-gray-600 transition-colors">Dashboard</NuxtLink>
          <span>/</span>
          <span class="text-gray-600 font-medium">Blog Categories</span>
        </div>
      </template>
      <template #actions>
        <button
          data-guide="add-post-category-btn"
          type="button"
          @click="startAdd"
    ...
    <!-- Loading -->
    <AdminSpinner v-if="loading" label="Loading blog categories…" />
    ...
    <!-- List -->
    <div v-else data-guide="post-category-list" class="bg-white rounded-2xl border border-gray-200/70 shadow-sm divide-y divide-gray-100">
      <div
        v-for="cat in categories"
        :key="cat.id"
        class="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 hover:bg-gray-50/50 transition-colors"
      >
        <!-- Gradient preview -->
        <div
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-gray-500 mt-0.5 truncate">
            {{ cat.posts_count ?? 0 }} published {{ (cat.posts_count ?? 0) === 1 ? 'post' : 'posts' }}
          </p>
        </div>
    ...
    <ConfirmDeleteModal
      ...
      :message="`Are you sure you want to delete &quot;${deleteTarget?.name}&quot;? Posts in this category will be left uncategorized.`"
      :deleting="deleting"
...
import type { PaginatedPostCategories, PostCategory } from '~/types/post'
...
async function load() {
  ...
  try {
    ...
    const res = await $apiFetch<PaginatedPostCategories>('/post-categories', { query })
...
function startEdit(cat: PostCategory) {
  editing.value = cat
  form.name = cat.name
  ...
  try {
    ...
    if (hasFile) {
      ...
      const url = editing.value
        ? `/post-categories/${editing.value.id}`
        : '/post-categories'
      ...
      if (editing.value) {
        ...
        await $apiFetch('/post-categories', { method: 'POST', body })
      }
    }

    // Clear image on existing category when the user removed it without uploading a new one
    if (editing.value && imageCleared.value && !hasFile && existingImageUrl.value) {
      await $apiFetch(`/post-categories/${editing.value.id}/image`, { method: 'DELETE' })
    }
...
async function executeDelete() {
  ...
  deleting.value = true
  try {
    await $apiFetch(`/post-categories/${deleteTarget.value.id}`, { method: 'DELETE' })
