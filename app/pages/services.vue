<template>
  <div class="min-h-screen flex flex-col bg-gray-50">
    ...
    <!-- Service groups -->
    <section v-for="(group, gi) in groups" :key="gi" class="border-b border-gray-100" :class="gi % 2 === 0 ? 'bg-white' : 'bg-gray-50'">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
        <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-12 max-w-xl">{{ group.heading }}</h2>
        ...
        <div class="space-y-14 sm:space-y-20">
          ...
          >
            ...
            >
              <p v-if="item.eyebrow" class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-400 mb-3">{{ item.eyebrow }}</p>
              <h3 class="text-xl sm:text-2xl font-bold text-gray-900 mb-3 leading-tight">{{ item.title }}</h3>
              <p class="text-sm sm:text-base text-gray-600 leading-relaxed mb-5">{{ item.description }}</p>
              <NuxtLink
                v-if="item.cta_label"
                :to="item.cta_link || '#'"
                class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-600 hover:text-primary-700 border-b border-primary-600/30 hover:border-primary-700 transition-colors pb-0.5"
              >
                {{ item.cta_label }}
...
const services = computed(() => siteConfig.value?.services_page)

const heroLabel = computed(() => services.value?.header?.label || 'OUR SERVICES')
const heroHeading = computed(() => services.value?.header?.heading || 'Services built around you')
const heroSubtitle = computed(() => services.value?.header?.subtitle || '')
...
const primaryCta = computed(() => services.value?.header?.primary_cta || null)
const secondaryCta = computed(() => services.value?.header?.secondary_cta || null)

const summary = computed(() => services.value?.summary?.items ?? [])
const stats = computed(() => services.value?.stats?.items ?? [])
const groups = computed(() => services.value?.groups ?? [])
const bottomCta = computed(() => services.value?.cta || null)

useHead({
  title: computed(() => `${services.value?.header?.label || 'Services'} – ${siteConfig.value?.site_name || ''}`),
  meta: [
    { name: 'description', content: computed(() => services.value?.header?.subtitle || '') },
  ],
})
</script>
